...
const scalePopup = (translate, scale) => {
    ...
    const popupCard = d3.select(".card-popup"),
        ...
        containerSize = [container.node().offsetWidth, container.node().offsetHeight],
        popSize = [popupCard.node().offsetWidth, popupCard.node().offsetHeight];
...
const zoomed = (datamap) => {
    ...
    datamap.svg.selectAll("g").style("stroke-width", 1.5 / limitBound.scale + "px");
    ...
    datamap.svg.selectAll("g").attr("transform", "translate(" + limitBound.translate + ")scale(" + limitBound.scale + ")");

    // scale label text
    datamap.svg.selectAll("text").style("font-size", 12 / limitBound.scale + "px");
...
const arcPopup = (data, position) => {
    ...
    if (popup.selectAll(".popup__close-btn").empty()) {
        popup.append("a").attr("class", "popup__close-btn cursor").append("span").html("&times;");
    }
...
const createGlows = (map) => {
    ...
    filter.append("feGaussianBlur").attr("class", "blur").attr("stdDeviation", "2.5").attr("result", "coloredBlur");
...
const mapLines = (map, data) => {
    ...
    enableGlow && map.svg.selectAll(".datamaps-arc").style("filter", "url(#glow)");
};
                ...
                if (showLabel) {
                    ...
                    const labelDestinationExists = tempLabels.find(
                        (label) =>
                            ...
                            label.coordinates[0] === arcData[i].destination.coordinates[0] &&
                            label.coordinates[1] === arcData[i].destination.coordinates[1]
                    );
